---
name: dev-kit-iteration
description: Define and enforce the repo-native iteration contract for agents.
---

## Purpose
Define and enforce the repo-native iteration contract for agents.
...
## Config
- `iteration.output_type` (default: `markdown`)
- `iteration.workflow_dir` (default: `.udx/dev.kit/workflows/<task-id>/workflow.md`)
...
- `workflow.bounded_work` must include: max_steps_per_iteration, max_files_per_step, max_new_files_per_iteration, max_move_operations_per_step, extract_child_workflow_if_any_exceeded.
...
## Output Rules
- Allowed outputs: request, command, artifact, workflow.
- Use iteration stages as the structure for any plan output.
...
- Any step exceeds bounded-work limits.
- Task has distinct deliverables that can be independently reviewed.
- Task spans multiple domains likely to exceed one-exec scope.
- Task needs commands outside the allowed command surface or extra validation.
...
- Parse input/prompt
- Validate intent and constraints
- Execute bounded work (read/write/execute)
- Process results
- Run tests/validation (when applicable)
- Prepare response (terse summary + next action)
...
- `README.md`
- `references/iteration-stages.md`
- `manifests/iteration-manifest.md`
