...
assert_file_exists "$DEV_KIT_HOME/src/configs/learning-workflows.yaml" "installs learning workflows catalog"
...
print_block "documented shell explore" "$explore_output"
assert_contains "$explore_output" "dev.kit explore" "explore command is available"
assert_contains "$explore_output" "what it is: library-cli" "explore reports repo identity"
assert_contains "$explore_output" "tools: git, gh, npm, docker" "explore reports operating tools"
assert_contains "$explore_output" "formats: yml, yaml" "explore reports operating formats"
assert_contains "$explore_output" "local repos: git/udx" "explore reports local knowledge root"
assert_contains "$explore_output" "remote org: github.com/udx" "explore reports remote knowledge root"
assert_contains "$explore_output" "dev.kit bridge --json" "explore reports typical workflows"

explore_json="$(cd "$DOCUMENTED_SHELL_REPO" && dev.kit explore --json)"
print_block "documented shell explore json" "$explore_json"
...
assert_contains "$explore_json" "\"operating_surface\": { \"tools\": [\"git\", \"gh\", \"npm\", \"docker\"]" "explore json reports operating tools"
assert_contains "$explore_json" "\"knowledge_sources\": [" "explore json reports knowledge sources"
assert_contains "$explore_json" "\"responsibility_split\": {" "explore json reports responsibility split"

self_explore="$(cd "$REPO_DIR" && dev.kit explore)"
print_block "self explore" "$self_explore"
assert_contains "$self_explore" "repo: dev.kit" "self explore reports the dev.kit repo"
assert_contains "$self_explore" "priority refs:" "self explore reports priority refs for the installed repo"
assert_contains "$self_explore" "src/configs/development-practices.yaml" "self explore highlights the repo practices catalog"
...
assert_contains "$bridge_json" "\"priority_refs\": [" "bridge json reports priority refs"
assert_contains "$bridge_json" "\"knowledge_base\": { \"local_repos_root\": \"git/udx\", \"remote_org_root\": \"github.com/udx\" }" "bridge json reports knowledge hierarchy"
assert_contains "$bridge_json" "\"knowledge_sources\": [" "bridge json reports knowledge sources"
assert_contains "$bridge_json" "\"operating_surface\": { \"tools\": [\"git\", \"gh\", \"npm\", \"docker\"]" "bridge json reports operating surface"
assert_contains "$bridge_json" "\"responsibility_split\": {" "bridge json reports responsibility split"
...
assert_contains "$learn_output" "wiki:" "learn reports wiki destination"
assert_contains "$learn_output" "slack:" "learn reports slack destination"
...
assert_contains "$learn_json" "\"id\": \"pr-lessons\"" "learn json reports workflow id"
...
assert_contains "$learn_json" "\"knowledge_sources\": [" "learn json reports knowledge sources"
assert_contains "$learn_json" "\"responsibility_split\": {" "learn json reports responsibility split"

help_output="$(dev.kit help)"
assert_contains "$help_output" "audit" "help discovers audit dynamically"
assert_contains "$help_output" "explore" "help discovers explore dynamically"
...
assert_contains "$help_output" "learn" "help discovers learn dynamically"
