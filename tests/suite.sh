...
if should_run "home"; then
  ...
  assert_contains "$default_output" "[read first]" "default landing renders read-first section"
  assert_contains "$default_output" "[do next]" "default landing renders do-next section"
  ...
  assert_contains "$default_json" "\"priority_refs\": [" "default json reports priority refs"
  assert_contains "$default_json" "\"entrypoints\": {" "default json reports entrypoints"
  ...
  assert_contains "$default_json" "\"next_git_action\":" "default json reports next git action"

  nested_output="$(cd "$NESTED_REPO_DIR" && dev.kit)"
  print_block "nested output" "$nested_output"
  assert_contains "$nested_output" "./README.md" "default landing resolves canonical refs from nested dir"
  ...
  assert_contains "$non_repo_output" "Navigate into a repo, then run dev.kit again." "default landing guides non repo users"
fi
...
if should_run "explore"; then
  ...
  assert_contains "$explore_json" "\"repo_kind\": \"repo\"" "explore json reports repo kind"
  assert_contains "$explore_json" "\"archetype\": \"library-cli\"" "explore json reports primary archetype"
  assert_contains "$explore_json" "\"profile\": \"shell\"" "explore json reports primary profile"
  assert_contains "$explore_json" "\"markers\": [" "explore json reports repo markers"
  assert_contains "$explore_json" "\"entrypoints\": {" "explore json reports entrypoints"
...
if should_run "action"; then
  ...
  assert_contains "$simple_action_json" "\"repo_kind\": \"repo\"" "action json reports repo kind"
  ...
  assert_contains "$simple_action_json" "\"entrypoints\": {" "action json reports entrypoints"
  assert_contains "$simple_action_json" "\"next_git_action\": \"\"" "action json reports empty next git action for non-git repo"
...
if should_run "git-action"; then
  ...
  assert_contains "$git_action_json" "\"next_git_action\": \"Current branch main has no upstream" "action json reports top-level next git action"
...
if should_run "repo-family" && [ "$TEST_MODE" = "full" ]; then
  ...
  assert_not_contains "$wordpress_explore_json" "\"./readme.md\"" "wordpress explore deduplicates case-equivalent refs"
