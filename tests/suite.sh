...
assert_contains "$default_output" "dev.kit explore" "default command runs explore"
assert_contains "$default_output" "what it is: library-cli" "default explore reports repo identity"
...
assert_contains "$simple_action" "git workflow: unavailable" "action reports missing git workflow cleanly"
...
git_action_ci="$(cd "$GIT_REPO" && dev.kit action --ci)"
assert_contains "$git_action_ci" "mode: ci" "action ci mode is available"

git_action_pr="$(cd "$GIT_REPO" && dev.kit action --pr)"
assert_contains "$git_action_pr" "mode: pr" "action pr mode is available"
...
refresh_output="$(cd "$SAVE_REPO" && dev.kit action --refresh-context --yes)"
print_block "refresh output" "$refresh_output"
assert_contains "$refresh_output" "context refresh: completed" "action reports context refresh"
assert_file_exists "$SAVE_REPO/.udx/dev.kit/todo.md" "action refresh writes todo context"
assert_file_exists "$SAVE_REPO/.udx/dev.kit/context.md" "action refresh writes context summary"
assert_file_exists "$SAVE_REPO/.udx/dev.kit/refs.md" "action refresh writes refs summary"

saved_todo="$(cat "$SAVE_REPO/.udx/dev.kit/todo.md")"
assert_contains "$saved_todo" "Run verification: \`bash tests/run.sh\`" "context refresh writes canonical verification guidance"

saved_refs="$(cat "$SAVE_REPO/.udx/dev.kit/refs.md")"
assert_contains "$saved_refs" "Migrated Legacy Review Notes" "context refresh migrates legacy tmp notes into refs"
assert_contains "$saved_refs" "keep a saved context for the next session" "context refresh preserves legacy note content"

saved_action_json="$(cd "$SAVE_REPO" && dev.kit action --json)"
assert_contains "$saved_action_json" "\"present\": true" "action json reports saved context when present"
assert_contains "$saved_action_json" "\"./.udx/dev.kit/todo.md\"" "action json points to saved todo context"
...
assert_contains "$action_completion_list" " --refresh-context " "action completion lists refresh flag"
assert_contains "$action_completion_list" " --pr " "action completion lists pr mode"
